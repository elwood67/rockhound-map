...
self.addEventListener('fetch', (e) => {
  ...
  // routes.json gets the same treatment: it's regenerated by fetch_routes.py,
  // so cache-first would pin you to a stale bundle forever. Network-first means
  // a fresh pull when online, and the last good copy when you're out at the river.
  const isRoutes = url.pathname.endsWith('routes.json');
  if (url.origin === self.location.origin && (isHTML || isRoutes)) {
    e.respondWith(
      fetch(e.request).then((res) => {
        const copy = res.clone();
        caches.open(SHELL).then((c) => c.put(isRoutes ? './routes.json' : './index.html', copy));
        return res;
      }).catch(() => caches.match(e.request).then((hit) =>
        hit || caches.match(isRoutes ? './routes.json' : './index.html')))
    );
    return;
  }
